...
class FileSystemDetailsReader
{
    ...
    public function getUserMods()
    {
        ...
        $usermods = array_map(fn($usermod) => substr((string) $usermod, strlen($this->settings->getBaseDirectory())),
            $usermods);
    ...
    public function getDangerousTools()
    {
        ...
        $dangerousTools = array_map(fn($dangerousTool) => substr((string) $dangerousTool, strlen($this->settings->getBaseDirectory())),
            $dangerousTools);
    ...
    public function getReceiptFiles()
    {
        ...
        $receiptFiles = array_map(fn($receiptFile) => basename((string) $receiptFile),
            $receiptFiles);
